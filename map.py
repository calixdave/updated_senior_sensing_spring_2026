...
def rotation_to_facing(rotation_ccw_deg):
    """
    Same convention as previous map code:
        0   -> UP
        90  -> RIGHT
        180 -> DOWN
        270 -> LEFT
...
def convert_object_grid_to_biggrid_perspective(local_object_3x3, rot_steps):
    ...
    But after map matching, the color grid may be rotated to match BIG_GRID.
...
def overlay_objects_on_matched_window(matched_biggrid_window, object_biggrid_perspective):
    ...
    matched_biggrid_window:
        the actual 3x3 floor window from BIG_GRID
...
def build_compact_17char(matched_biggrid_window, object_biggrid_perspective, final_direction):
    """
    17-character compact output:
        8 surrounding cells x 2 chars each = 16
        final direction char = 1
